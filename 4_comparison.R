...
running_window_predictions <- running_window_predictions %>% left_join(
...
results_twcrps %>% 
  ...
  ggplot(aes(x = q, y = twCRPS, col = model)) + geom_line() +
  xlab("Quantile") +
  theme_classic() + 
  theme(axis.text=element_text(size=14),axis.title=element_text(size=14), plot.title = element_text(size = 14,hjust = 0.5)) +
  xlab("twCRPSS") +
...
results %>% 
  ...
  scale_fill_manual(name = "EMOS type", values = c("Separated (S-)" = "#ffa300", "Continuous (C-)" = "#0bb4ff")) +
  theme_classic() + 
  theme(axis.text=element_text(size=14),axis.title=element_text(size=14), plot.title = element_text(size = 14,hjust = 0.5)) +  ylab("p Variogram score (p = 0.5)")
...
results_twcrps %>% 
  ...
  pivot_longer(contains("EMOS"), names_to = "model", values_to = "twCRPS") %>%
  filter(model != "Separated EMOS - Running window") %>%
  ggplot(aes(x = t, y = twCRPS, col = model)) + geom_line(linewidth = 1.2) +
  ylab("twCRPSS") +
  theme_classic() + 
  theme(axis.text=element_text(size=14),axis.title=element_text(size=14), plot.title = element_text(size = 14,hjust = 0.5)) +
  ggtitle("twCRPSS as a function of lead time") + 
  xlab("Lead time (hours") + facet_wrap(~lead_time)
...
results_twcrps %>% 
  ...
  ggplot(aes(x = t, y = twCRPS, col = type, linetype = training)) + geom_line(linewidth = 1.2) +
  ylab("twCRPSS") +
  scale_color_manual(name = "EMOS type", values = c("Separated (S-)" = "#000000", "Continuous (C-)" = "#E69F00")) +
